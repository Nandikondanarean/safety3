...
function updateSafetyHeatmap(lat, lng) {
    ...
    // Score model: Hyderabad zones (mock, extendable with real crime API)
    let score = 78, statusText = '🟡 MODERATE — Stay Alert', locText = 'Hyderabad';
    
    if (lat && lng) {
        if (lat >= 17.39 && lat <= 17.45 && lng >= 78.40 && lng <= 78.46) {
            score = 88; statusText = '🛡️ HIGH SAFETY — SHE Teams Patrolling'; locText = 'Banjara Hills / Jubilee Hills';
        } else if (lat >= 17.44 && lat <= 17.50) {
            score = 72; statusText = '🟡 MODERATE — Well-lit Area'; locText = 'Ameerpet / Begumpet';
        } else if (lat > 17.55) {
            score = 48; statusText = '🔴 HIGH RISK — Avoid After Dark'; locText = 'Northern Outskirts';
        } else if (lat < 17.30) {
            score = 52; statusText = '🟠 CAUTION — Call Guardian'; locText = 'South Hyderabad';
        }
    }
...
function startJourneyTimer() {
    ...
    const dest = (document.getElementById('journey-dest')?.value || '').trim() || 'Destination';
    ...
    if (startBtn) { startBtn.innerHTML = '<span>❌ Cancel Journey</span>'; startBtn.classList.add('active-journey'); }
    if (safeBtn) safeBtn.classList.remove('hidden');
    if (ringLabel) ringLabel.textContent = 'ACTIVE';
    if (ringFill) { ringFill.classList.remove('warning','danger'); ringFill.classList.add('safe'); }
    ...
    journeyTimerInterval = setInterval(() => {
        ...
        if (pct < 0.25) {
            ...
            // Vibrate every 10 seconds in danger zone
...
async function pinCommunityAlert() {
    ...
    try {
        ...
        let saved = false;
        try {
            const resp = await fetch('/api/community/pin', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function loadCommunityReports() {
    ...
    if (!window.communityMap && typeof L !== 'undefined') {
        ...
        if (mapEl) {
            ...
            }).setView([17.385044, 78.486671], 13); // Default to Hyderabad
            
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                maxZoom: 19
            }).addTo(window.communityMap);
            
            L.control.zoom({ position: 'bottomright' }).addTo(window.communityMap);
        }
    }

    // Try server API first (shared Firestore data)
    try {
        const resp = await fetch('/api/community/pins');
        if (resp.ok) {
